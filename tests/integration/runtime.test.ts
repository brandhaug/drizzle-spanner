...
describe('read-only transactions', () => {
  ...
  it('supports a readTimestamp bound', async () => {
    ...
    await db.insert(singers).values({ id: 'rt-1', name: 'Ada', updatedAt: commitTimestamp() });
    ...
    const seen = await db.transaction(
      async (tx) => tx.select().from(singers).where(inArray(singers.id, ['rt-1', 'rt-2'])),
  ...
  it('throws a typed error on DML inside the snapshot', async () => {
    ...
    await expect(
      db.transaction(
        async (tx) => {
          await (tx as unknown as SpannerDatabase).insert(singers).values({ id: 'x', name: 'y' });
        },
        { readOnly: true },
      ),
...
describe('single-use bounded reads (withStaleness)', () => {
  ...
  it('runs a readTimestamp bounded read', async () => {
    const { db } = harness;
    await db.insert(singers).values({ id: 'su-2', name: 'Ada', updatedAt: commitTimestamp() });
