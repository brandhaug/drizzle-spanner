...
describe('column type round-trips', () => {
  it('round-trips every column type with values', async () => {
    ...
    const [selected] = await db.select().from(everyType).where(eq(everyType.id, 'row-1'));
  ...
  it('binds null for every nullable column (schema-derived type hints)', async () => {
    ...
    const [selected] = await db.select().from(everyType).where(eq(everyType.id, 'row-nulls'));
  ...
  it('binds empty arrays (the untyped-empty-array sharp edge)', async () => {
    ...
    const [selected] = await db.select().from(everyType).where(eq(everyType.id, 'row-empty'));
...
describe('commit timestamps', () => {
  it('writes the commitTimestamp() sentinel and reads a real timestamp after commit', async () => {
    ...
    await db.insert(events).values({ id: 'e1', name: 'signup', createdAt: commitTimestamp() });
    ...
    await db.update(events).set({ createdAt: commitTimestamp() }).where(eq(events.id, 'e1'));
    const [updated] = await db.select().from(events).where(eq(events.id, 'e1'));
    expect(updated!.createdAt!.getTime()).toBeGreaterThanOrEqual(event!.createdAt!.getTime());
  });
});
