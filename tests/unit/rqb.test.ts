...
import { drizzle, int64, primaryKey, spannerTable, string, timestamp } from '../../src/index.js';
...
describe('relational queries: golden SQL', () => {
  ...
  it('findFirst compiles to limit 1', () => {
    ...
    expect(text).toBe('select `d0`.`id` as `id`, `d0`.`name` as `name` from `singers` as `d0` limit @p0');
