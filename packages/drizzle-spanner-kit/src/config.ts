...
function validateDatabase(database: unknown): SpannerKitDatabaseConfig | undefined {
  ...
  for (const key of ['project', 'instance', 'database'] as const) {
    if (typeof candidate[key] !== 'string' || candidate[key].length === 0) {
      throw new Error(`drizzle-spanner-kit config: database.${key} must be a non-empty string`);
    }
  }
 ...
 * (Node >= 22.18, Bun).
 */
export async function loadConfig(configPath: string): Promise<ResolvedSpannerKitConfig> {
  ...
  const schemaInput =
    typeof config.schema === 'string' ? [config.schema] : (config.schema as string[] | undefined);
  if (!schemaInput || schemaInput.length === 0) {
    throw new Error('drizzle-spanner-kit config: schema must name at least one schema module path');
  }
  return {
