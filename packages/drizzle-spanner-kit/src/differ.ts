...
function indexEntities(ddl: SpannerEntity[]): EntityIndex {
  ...
  for (const column of buckets.columns) {
    const tableColumns =
      index.columns.get(column.table) ?? new Map<string, ColumnEntity>()
...
function renameTableInIndex(
  ...
  if (tableColumns) {
    ...
    index.columns.set(
      newName,
      new Map(
        [...tableColumns].map(([name, column]) => [name, { ...column, table: newName }])
      )
    )
  }
...
export async function diffSnapshots(
  ...
  let createdTableNames = [...cur.tables.keys()].filter(
  ...
  const stillDropped = [...prev.tables.values()].filter(
    (table) => !cur.tables.has(table.name)
  )
  ...
  for (const table of orderTablesParentsFirst(createdTables, curFksByTable)) {
    ...
    if (!pk) {
      throw new Error(
        `drizzle-spanner-kit: snapshot has no primary key for "${table.name}"`
      )
    }
  ...
  for (const [tableName, curTable] of cur.tables) {
    ...
    for (const [columnName, curColumn] of curColumns) {
      ...
      if (
        prevColumn.type !== curColumn.type &&
        !typeChangeAllowed(prevColumn.type, curColumn.type)
      ) {
        diagnostics.push({
      ...
      if (
        prevColumn.type !== curColumn.type ||
        prevColumn.notNull !== curColumn.notNull
      ) {
  ...
  for (const [key, curFk] of cur.fks) {
    ...
    if (!prevFk || !equalJson(prevFk, curFk)) {
      buckets.addConstraints.push(
        addConstraintSql(curFk.table, foreignKeyConstraintSql(curFk))
      )
    }
  }
  ...
  for (const [key, curCheck] of cur.checks) {
    ...
    if (!prevCheck || !equalJson(prevCheck, curCheck)) {
      buckets.addConstraints.push(
        addConstraintSql(curCheck.table, checkConstraintSql(curCheck))
      )
    }
  }
