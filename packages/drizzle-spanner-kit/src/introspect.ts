...
export async function introspectDatabase(
  ...
  for (const tableRow of tables) {
    ...
    for (const constraintRow of constraints) {
      ...
      const foreignTable = uniqueUsage[0]
        ? text(uniqueUsage[0], 'TABLE_NAME')
        : tableName
      const entity: ForeignKeyEntity = {
        ...
        foreignColumns: fkUsage.map((row) => {
          ...
          const match = uniqueUsage.find(
            (unique) => Number(unique['ORDINAL_POSITION']) === position
          )
